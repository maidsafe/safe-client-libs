...
pub fn mutate_mdata_entries<T: 'static>(client: &Client<T>,
                                        name: XorName,
                                        tag: u64,
            ...
            .or_else(move |error| match error {
                CoreError::RoutingClientError(ClientError::InvalidEntryActions(errors)) => {
                    if attempts < MAX_ATTEMPTS {
                        ...
                        Err(CoreError::RoutingClientError(ClientError::InvalidEntryActions(errors)))
                    }
                }
...
pub fn set_mdata_user_permissions<T: 'static>(client: &Client<T>,
                                              name: XorName,
                                              tag: u64,
                                              user: User,
    ...
    future::loop_fn(state, move |(attempts, version)| {
        client.set_mdata_user_permissions(name, tag, user, permissions, version)
        .map(|_| Loop::Break(()))
        .or_else(move |error| match error {
...
pub fn del_mdata_user_permissions<T: 'static>(client: &Client<T>,
                                              name: XorName,
                                              tag: u64,
                                              user: User,
                                              version: u64)
    ...
    future::loop_fn(state, move |(attempts, version)| {
        client.del_mdata_user_permissions(name, tag, user, version)
        .map(|_| Loop::Break(()))
        .or_else(move |error| match error {
...
fn update_mdata<T: 'static>(client: &Client<T>, data: MutableData) -> Box<CoreFuture<()>> {
    ...
    f0.join3(f1, f2)
        .and_then(move |(entries, permissions, version)| {
            update_mdata_permissions(&client2,
                                     *data.name(),
                                     data.tag(),
                                     permissions,
...
fn update_mdata_entries<T: 'static>(client: &Client<T>,
                                    name: XorName,
                                    tag: u64,
    ...
    let actions = desired_entries
        .into_iter()
        .filter_map(|(key, value)| if let Some(current_value) = current_entries.get(&key) {
...
fn update_mdata_permissions<T: 'static>(client: &Client<T>,
                                        name: XorName,
                                        tag: u64,
    ...
    let permissions: Vec<_> = desired_permissions
        .into_iter()
        .map(|(user, desired_set)| if let Some(current_set) = current_permissions.get(&user) {
    ...
    future::loop_fn(state,
                    move |(client, mut permissions, version)| if let Some((user, set)) =
        permissions.pop() {
                        let f =
                            set_mdata_user_permissions(&client, name, tag, user, set, version)
                                .map(move |_| {
                                         Loop::Continue((client, permissions, version + 1))
                                     });
                        Either::A(f)
                    } else {
...
fn fix_entry_actions(actions: BTreeMap<Vec<u8>, EntryAction>,
...
fn union_permission_sets(a: &PermissionSet, b: &PermissionSet) -> PermissionSet {
    let actions = [Action::Insert,
                   Action::Update,
                   Action::Delete,
                   Action::ManagePermissions];
    actions
        .into_iter()
        .fold(PermissionSet::new(),
              |set, &action| match (a.is_allowed(action), b.is_allowed(action)) {
...
mod tests {
    ...
    fn test_fix_entry_actions() {
        ...
        let _ = actions.insert(vec![0],
                               EntryAction::Ins(Value {
                                                    content: vec![0],
                                                    entry_version: 0,
                                                }));
        let _ = actions.insert(vec![1],
                               EntryAction::Ins(Value {
                                                    content: vec![1],
                                                    entry_version: 0,
                                                }));
        let _ = actions.insert(vec![2],
                               EntryAction::Update(Value {
                                                       content: vec![2],
                                                       entry_version: 1,
                                                   }));
        let _ = actions.insert(vec![3],
                               EntryAction::Update(Value {
                                                       content: vec![3],
                                                       entry_version: 1,
                                                   }));
        let _ = actions.insert(vec![4],
                               EntryAction::Update(Value {
                                                       content: vec![4],
                                                       entry_version: 1,
                                                   }));
        ...
        // 0: insert is OK.
        assert_eq!(*unwrap!(actions.get([0].as_ref())),
                   EntryAction::Ins(Value {
                                        content: vec![0],
                                        entry_version: 0,
                                    }));
        ...
        assert_eq!(*unwrap!(actions.get([1].as_ref())),
                   EntryAction::Update(Value {
                                           content: vec![1],
                                           entry_version: 3,
                                       }));
        ...
        // 2: update is OK.
        assert_eq!(*unwrap!(actions.get([2].as_ref())),
                   EntryAction::Update(Value {
                                           content: vec![2],
                                           entry_version: 1,
                                       }));
        ...
        assert_eq!(*unwrap!(actions.get([3].as_ref())),
                   EntryAction::Ins(Value {
                                        content: vec![3],
                                        entry_version: 1,
                                    }));
        ...
        assert_eq!(*unwrap!(actions.get([4].as_ref())),
                   EntryAction::Update(Value {
                                           content: vec![4],
                                           entry_version: 3,
                                       }));
...
mod tests_with_mock_routing {
    ...
    fn put_mdata_with_recovery() {
        random_client(|client| {
            ...
            let data0 = unwrap!(MutableData::new(name, tag, permissions, entries, owners.clone()));
            ...
            client
                ...
                .then(move |res| {
                    ...
                    assert_eq!(*unwrap!(entries.get([0].as_ref())),
                               Value {
                                   content: vec![0, 1],
                                   entry_version: 1,
                               });
                    assert_eq!(*unwrap!(entries.get([1].as_ref())),
                               Value {
                                   content: vec![1, 0],
                                   entry_version: 1,
                               });
                ...
                .then(move |res| {
                    ...
                    assert_eq!(*unwrap!(permissions.get(&User::Anyone)),
                               PermissionSet::new()
                                   .allow(Action::Insert)
                                   .allow(Action::Update));
                    assert_eq!(*unwrap!(permissions.get(&user)),
        ...
        random_client(|client| {
            ...
            client
                ...
                .then(move |res| {
                    ...
                    assert_eq!(*unwrap!(entries.get([2].as_ref())),
                               Value {
                                   content: vec![2, 0],
                                   entry_version: 1,
                               });
                    assert_eq!(*unwrap!(entries.get([3].as_ref())),
                               Value {
                                   content: vec![3],
                                   entry_version: 1,
                               });
                    assert_eq!(*unwrap!(entries.get([4].as_ref())),
                               Value {
                                   content: vec![4, 0],
                                   entry_version: 1,
                               });
                    assert_eq!(*unwrap!(entries.get([5].as_ref())),
                               Value {
                                   content: vec![],
                                   entry_version: 1,
                               });
                    assert!(entries.get([6].as_ref()).is_none());
                    assert_eq!(*unwrap!(entries.get([7].as_ref())),
                               Value {
                                   content: vec![],
                                   entry_version: 1,
                               });
