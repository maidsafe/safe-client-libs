...
use sn_data_types::{
    ...
    MsgSender, PublicKey, QueryResponse, Signature, TransferValidated,
};
...
impl ConnectionManager {
    ...
    /// Send a Command `Message` to the network without awaiting for a response.
    pub async fn send_cmd(&mut self, msg: &Message) -> Result<(), ClientError> {
        info!("Sending command message {:?} w/ id: {:?}", msg, msg.id());
        let msg_bytes = self.serialise_in_envelope(msg, None)?;
        self.send_signed_cmd(msg_bytes).await
    }

    /// Send bytes of a signed Command `Message` to the network without awaiting for a response.
    pub async fn send_signed_cmd(&mut self, msg_bytes: Bytes) -> Result<(), ClientError> {
    ...
    pub async fn send_query(&mut self, msg: &Message) -> Result<QueryResponse, ClientError> {
        ...
        let msg_bytes = self.serialise_in_envelope(msg, None)?;
        self.send_signed_query(msg_bytes).await
    }

    /// Send bytes of a signed Query `Message` to the network awaiting for the response.
    ...
    // If an origin's pk and corresponding message signature is not provided,
    // currently set keypair will be used as the origin and to sign the message.
    pub(crate) fn serialise_in_envelope(
        &self,
        message: &Message,
        origin: Option<(PublicKey, Signature)>,
    ) -> Result<Bytes, ClientError> {
        trace!("Putting message in envelope: {:?}", message);
        let (pk, sign) = match origin {
            Some(origin) => origin,
            None => (
                self.keypair.public_key(),
                self.keypair.sign(&serialize(message)?),
            ),
        };

        let envelope = MsgEnvelope {
            message: message.clone(),
            origin: MsgSender::client(pk, sign)?,
